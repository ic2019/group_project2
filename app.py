...
def index():
    ...
    DATABASE_URI = f"postgresql://postgres:1qazxsw2@localhost:5432/fortune500_db"
...
def fortune_api():
   ...
   mydata_final = {
       ...
       "sector":mydata["Sector"].values.tolist()
   }
   return jsonify(mydata_final)

@app.route('/fortune_api_500')
def fortune_api_500():
   # Connecting to postgreSQL database
   DATABASE_URI = f"postgresql://postgres:1qazxsw2@localhost:5432/fortune500_db"
   engine = create_engine(DATABASE_URI)
   mydata = pd.read_sql_query('select * from fortune500', con=engine)
   mydata_final = {
       "ticks": mydata["Symbol"].values.tolist(),
       "revenue":mydata["Revenues"].values.tolist(),
       "profit":(mydata["Profits"]/100).values.tolist(),
